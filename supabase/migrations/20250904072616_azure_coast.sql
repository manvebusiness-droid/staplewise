...
-- Create product price history table with correct syntax
...
CREATE INDEX IF NOT EXISTS idx_price_history_product_date ON product_price_history(product_id, changed_at);
...
SELECT 
    ...
    created_at
FROM products
WHERE id NOT IN (SELECT DISTINCT product_id FROM product_price_history WHERE product_id IS NOT NULL);
...
-- Create function to get price history for all products by seller
CREATE OR REPLACE FUNCTION get_seller_price_history(
    p_seller_id UUID,
    p_days INTEGER DEFAULT 30
)
RETURNS TABLE (
...
BEGIN
    ...
    SELECT 
        ...
        DATE(ph.changed_at) as date,
        ph.price_per_kg as price,
        ph.change_reason
    FROM products p
    JOIN product_price_history ph ON p.id = ph.product_id
    WHERE p.seller_id = p_seller_id
    AND ph.changed_at >= NOW() - INTERVAL '1 day' * p_days
