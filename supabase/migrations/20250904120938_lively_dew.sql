/*
  # Enhance Orders Table for Admin Dashboard

  1. New Columns
    - Add all missing fields for complete order management
...
SET 
  ...
  product_sku = COALESCE(product_sku, 'CSH-W320-' || LPAD((ROW_NUMBER() OVER (ORDER BY created_at))::TEXT, 6, '0')),
...
CREATE OR REPLACE FUNCTION populate_order_seller_info()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER populate_order_info_trigger
    ...
    FOR EACH ROW
    EXECUTE FUNCTION populate_order_seller_info();